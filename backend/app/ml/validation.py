...
def walk_forward_splits(n_samples: int, n_splits: int = 5, embargo: int = 0,
    ...
    The tail of the series is divided into `n_splits` equal contiguous test
    blocks. For each, training is everything before it minus the last
    `embargo` bars.
    """
    if n_splits < 1:
    ...
    # Reserve an initial training region, then split the remainder into folds.
